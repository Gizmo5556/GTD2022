...
function scr_get_target_boshy(targeting_mode, boshy_id){
	...
	switch(targeting_mode) {
		...
		case 0:
			...
			scr_boshy_collision_circle_list(x,y,radius,objApple,0,0,_list, boshy_id);
		...
		case 2:
			...
			if ds_list_size(_list) > 0 {
				...
				var target_dist = point_distance(boshy_id.x, boshy_id.y, target.x, target.y);
				for (var i = 0; i < ds_list_size(_list); i++) {
					//distance from boshy to new target
					var compared_dist = point_distance(boshy_id.x, boshy_id.y, _list[|i].x, _list[|i].y)
